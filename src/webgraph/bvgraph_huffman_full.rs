...
use crate::{ImmutableGraph, int2nat, nat2int, plain_webgraph::BVGraphPlain, properties::Properties, utils::{encodings::{UniversalCode, GammaCode}, huffman::{Huffman, HuffmanEncoder, HuffmanDecoderIterator}}};
...
> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let decoded_list = self.graph.as_ref().decode_list(self.curr as usize, self.ibs.clone(), Some(&mut self.window), &mut self.outd);
...
> 
{
    ...
    fn into_iter(self) -> Self::IntoIter {
        BVGraphNodeIterator {
            n: self.n,
            ibs: Rc::new(RefCell::new(BinaryReader::new(self.graph_memory.clone()))),
...
> {
    ...
    pub fn successors(&mut self, x: usize) -> Box<[usize]> {
        ...
        self.decode_list(x, self.graph_binary_wrapper.clone(), None, &mut [])
    }
    
    #[inline(always)]
    fn decode_list(&self, x: usize, decoder: Rc<RefCell<BinaryReader>>, window: Option<&mut Vec<Vec<usize>>>, outd: &mut [usize]) -> Box<[usize]> {
        ...
        let mut block_huff_iter = HuffmanDecoderIterator::default();
        ...
        if reference > 0 {
            ...
            if block_count != 0 {
                ...
                block_huff_iter = HuffmanDecoderIterator::new(decoder.clone(), 16, 16); // DUMMY VALUES. CHANGE BASED ON .PROPERTIES
            }
            ...
            let mut i = 0;
            while i < block_count {
                block.push(InBlockCoding::read_next(&mut block_huff_iter) as usize + if i == 0 {0} else {1});
                total += block[i];
        ...
        if extra_count > 0 && self.min_interval_len != 0 {
            ...
            if interval_count != 0 {
                ...
                left.push(nat2int(GammaCode::read_next(&mut decoder.borrow_mut(), self.zeta_k)) + x as i64);
                len.push(GammaCode::read_next(&mut decoder.borrow_mut(), self.zeta_k) as usize + self.min_interval_len);
                let mut prev = left[0] + len[0] as i64;  // Holds the last integer in the last interval
                extra_count -= len[0];

                let mut i = 1;
                while i < interval_count {
                    prev += GammaCode::read_next(&mut decoder.borrow_mut(), self.zeta_k) as i64 + 1;
        ...
        if extra_count > 0 {
            let mut residual_huff_iter = HuffmanDecoderIterator::new(decoder.clone(), 16, 16); // DUMMY VALUES. CHANGE BASED ON .PROPERTIES
            residual_list.push(x as i64 + nat2int(InResidualCoding::read_next(&mut residual_huff_iter)));
            let mut remaining = extra_count - 1;
            let mut curr_len = 1;

            while remaining > 0 {
                residual_list.push(residual_list[curr_len - 1] + InResidualCoding::read_next(&mut residual_huff_iter) as i64 + 1);
                curr_len += 1;
    ...
    fn diff_comp(
        ...
        if reference != 0 {
            ...
            if block_count > 0 {                
                for b in self.compression_vectors.blocks.borrow_mut().iter_mut().skip(1) {
                    *b -= 1;
                }

                ///////////////////////////////// HUFFMAN HERE /////////////////////////////////////
                
                let huff = HuffmanEncoder::build_huffman_zuck(&self.compression_vectors.blocks.borrow());
                HuffmanEncoder::write_huffman_zuck(&huff, graph_obs);
        ...
        if extra_count > 0 {
            ...
            if residual_count != 0 {
                let mut residual_vec = Vec::with_capacity(residual_count);

                prev = residual[0];
                // _t = self.write_residual(graph_obs, int2nat(prev as i64 - curr_node as i64) as usize)?;
                residual_vec.push(int2nat(prev as i64 - curr_node as i64) as usize);
                for i in 1..residual_count {
                    ...
                    residual_vec.push(residual[i] - prev - 1);
                    prev = residual[i];
                }

                ///////////////////////////////// HUFFMAN HERE /////////////////////////////////////
                
                let huff = HuffmanEncoder::build_huffman_zuck(&residual_vec);
                HuffmanEncoder::write_huffman_zuck(&huff, graph_obs);
            }
        }
